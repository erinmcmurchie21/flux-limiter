...
def adsorption_isotherm_1(pressure, mole_fraction):
    K_1 = 6.077 # adsorption equilibrium constant
    R = 8.314 # J/(mol*K)
    T = 308.15 # K
...
def adsorption_isotherm_2(pressure, mole_fraction):
    K_2 = 6.077 # adsorption equilibrium constant
    R = 8.314 # J/(mol*K)
    T = 308.15 # K
...
class AdsorptionColumn(object):
    ...
    def outlet_BC(self, results_vector):
        ...
        if self.outlet_type == "pressure":
            ...
            a = 1.75 * (1- self.bed_voidage) * outlet_values["outlet_density"] / (self.bed_voidage**3 * self.particle_diameter)
    ...
    def calculate_wall_values(self, results_vector):
        ...
        c = -np.abs(dPdz_walls[:])
        v_walls = np.array((-b + np.sqrt(b**2 - 4*a*c)) / (2*a)) # should have N + 1 values 
        ...
        P_walls = P_centres_and_ghost_cells[0:Nx+1] + dPdz_walls * (self.deltaZ[0:Nx+1]/2)
    ...
    def ODE_calculations(self, t, results_vector):
        ...
        Nx = int(self.num_cells)
        N_walls = Nx + 1  # number of wall values
        ...
        dydt = 1/ (self.deltaZ[1:-1] * P[:]) * (v_walls[1:]*P_walls[1:]*y_walls[1:] - v_walls[:Nx]*P_walls[:Nx]*(y_walls[:Nx])) - self.rho_b*self.R*self.T_column/self.bed_voidage * dn1dt[:] - y[:]/P[:] * dPdt[:]
